...
export default function EditGearPage() {
  ...
  useEffect(() => {
    ...
    setLocation(gear.data.location ?? "");
    setSpecs(toSpecRows(gear.data.specifications));
    setImages(gear.data.images.length > 0 ? gear.data.images : [""]);
...
function toSpecRows(specifications: GearItem["specifications"]): { key: string; value: string }[] {
